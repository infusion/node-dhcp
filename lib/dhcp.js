...
Server.prototype = {
  ...
  _selectAddress: function(clientMAC, req) {
    ...
    // Is there a option82 binding?
    const _relayAgentInformation = this.config('relayAgentInformation');
    console.log("Option82: %s", typeof _relayAgentInformation[req.options["82"]["remoteId"]]);
    if (typeof _relayAgentInformation === "function") {
      const relayAgentInformationResult = _relayAgentInformation(req.options["82"]["remoteId"], req.options["82"]["circuitId"]);
      if (relayAgentInformationResult)
        return relayAgentInformationResult;
    } else if (typeof _relayAgentInformation[req.options["82"]["remoteId"]] === "string") {
      return _relayAgentInformation[req.options["82"]["circuitId"]];
    } else if (typeof _relayAgentInformation[req.options["82"]["remoteId"]] === "object") {
      return _relayAgentInformation[req.options["82"]["remoteId"][req.options["82"]["circuitId"]]];
    }
