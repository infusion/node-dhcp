...
SeqBuffer.prototype = {
  ...
  getOptions: function() {
    ...
    while (this._r < buf.length) {
      ...
      if (opt === 0xff) { // End type
        ...
        if (opt in Options) {
          if (opt == 82) {
            options[opt] = this['get' + Options[opt].type](Options[opt].default);
          } else {
  ...
  getSubOptions(vendor) {

    const sub_options = {};
    const buf = this._data;

    while (this._r < buf.length - 1) {
        let opt = this.getUInt8();
        let len = this.getUInt8();
        ...
        if (opt == 0x01) {
          switch(vendor) {
            default:
              sub_options["circuitId"] = this.getASCII(len);
          }
        }
        ...
        if (opt == 0x02) {
          switch(vendor) {
            case "cisco":
              // Cisco Remote ID Type
              let sub_type = this.getUInt8();
              // Cisco Remote ID Length
              let sub_type_length = this.getUInt8();
              sub_options["remoteId"] = this.getASCII(sub_type_length);
              break;
            default:
              sub_options["remoteId"] = this.getASCII(len);
              break;
          }
        }
    }
